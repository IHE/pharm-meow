...
| D15 | **Bidirectional linkage between treatment lines and prescriptions** | Two use cases require opposite link directions: (1) Prescription-first — treatment line is derived from a prescription → `MedicationStatement.derivedFrom` → MedicationRequest. This works in R5. (2) Overview-first — a treatment line is the plan, and a prescription is administratively derived from it → `MedicationRequest.basedOn` should point to the treatment line, but R5 `basedOn` does not accept MedicationStatement. Note: R5 `MedicationStatement.basedOn` does not exist (removed from R4). | Options: (a) Use CarePlan as intermediary — prescription `basedOn` CarePlan, treatment line is part of that CarePlan. Clean but forces CarePlan even when not clinically needed. (b) Use `MedicationRequest.supportingInformation` → MedicationStatement. Works but semantically weak. (c) Define an extension on MedicationRequest for the originating treatment line. (d) Accept `derivedFrom` as bidirectional — treatment line references its prescriptions via `derivedFrom` regardless of which came first. This covers retrieval/display but not formal creation semantics. **This decision affects both MEOW and MPD and should be coordinated with the MPD group.** |
...
| *New — to create* | Bidirectional linkage treatment line ↔ prescription (overview-first vs. prescription-first) | D15 |
