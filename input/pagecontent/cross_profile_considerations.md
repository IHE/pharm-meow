...
2. Clinician reviews the overview and decides to prescribe → system uses MPD (PHARM-5) to submit the new prescription
3. The new prescription is incorporated into the medication overview (by the MEOW Responder, internally)
4. Next clinician who opens the patient sees the updated overview
...
These groupings are specified in [Actor Required Grouping](actor_required_grouping.html).
