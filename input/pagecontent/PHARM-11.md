...
The response is a `MedicationOverviewBundle` — a `Bundle` of type `document` — that carries authorship metadata via a `Composition`, and is suitable for handover, discharge summaries, and legal records. Unlike [PHARM-12](PHARM-12.html), which returns a live searchset of individual resources, this transaction returns a snapshot that is attributed and immutable.
...
<div>
<img alt="Figure: PHARM-11 Retrieve Medication Overview" src="PHARM-11.png" width="70%">
</div>
...
A single `MedicationOverviewBundle` resource is returned, conforming to [MedicationOverviewBundle](StructureDefinition-MedicationOverview.html).
...
- SHALL return a `MedicationOverviewBundle` conforming to [MedicationOverviewBundle](StructureDefinition-MedicationOverview.html) for a successful Pattern A request.
